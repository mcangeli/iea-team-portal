...
class LessonProgram(models.Model):
    ...
    group = models.ForeignKey(
        "portal.OrganizationGroup", on_delete=models.PROTECT, null=True, blank=True,
        related_name="lesson_programs",
    )
    ...
    def clean(self):
        super().clean()
        if self.default_capacity is not None and self.default_capacity < 1:
            raise ValidationError({"default_capacity": "Capacity must be at least 1."})
        if self.enrollment_opens and self.enrollment_closes and self.enrollment_closes < self.enrollment_opens:
            raise ValidationError("Enrollment close date cannot be before the open date.")
        if self.group_id:
            if self.group.team_id != self.team_id:
                raise ValidationError("Lesson program group must belong to the same organization.")
            if self.group.group_type != self.group.GroupType.PROGRAM:
                raise ValidationError("Lesson programs may only use an organization Program group.")
...
class LessonSeries(models.Model):
    ...
    def clean(self):
        ...
        if self.start_date and self.end_date and self.end_date < self.start_date:
            raise ValidationError("Series end date cannot be before the start date.")
        if self.instructor_id and self.instructor.team_id != self.program.team_id:
            raise ValidationError("Lesson series instructor must belong to the same organization.")
...
class LessonEnrollment(models.Model):
    ...
    def clean(self):
        ...
        if self.start_date and self.end_date and self.end_date < self.start_date:
            raise ValidationError("Enrollment end date cannot be before the start date.")

    def __str__(self):
        return f"{self.person} — {self.series}"
...
class LessonOccurrence(models.Model):
    ...
    def clean(self):
        ...
        if self.ends_at and self.ends_at <= self.starts_at:
            raise ValidationError("Lesson occurrence end time must be after its start time.")
        if self.capacity is not None and self.capacity < 1:
            raise ValidationError({"capacity": "Capacity must be at least 1."})
...
class LessonAttendanceRecord(models.Model):
    ...
    def clean(self):
        super().clean()
        if self.person_id and self.person.team_id != self.occurrence.series.program.team_id:
            raise ValidationError("Lesson attendance must remain within one organization.")
...
class LessonAssignment(models.Model):
    ...
    def clean(self):
        super().clean()
        team_id = self.occurrence.series.program.team_id
        if self.person_id and self.person.team_id != team_id:
            raise ValidationError("Lesson assignment person must belong to the same organization.")
        if self.horse_id and self.horse.team_id != team_id:
            raise ValidationError("Lesson assignment horse must belong to the same organization.")
        if self.role == self.Role.INSTRUCTOR and self.horse_id:
            raise ValidationError("Instructor assignments cannot have a horse assignment.")

    def __str__(self):
        return f"{self.person} — {self.get_role_display()} — {self.occurrence}"
