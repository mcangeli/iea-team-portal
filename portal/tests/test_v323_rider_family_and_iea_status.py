...
class V323RiderFamilyAndIEAStatusTests(TestCase):
    ...
    def test_link_existing_person_creates_legacy_and_canonical_parent_relationship(self):
        response = self.client.post(reverse("rider_guardian_link", args=[self.rider.pk]), {"person": self.parent_person.pk, "relationship": "Mother", "primary_contact": "on"})
    ...
    def test_unlink_ends_canonical_relationship_but_keeps_people_records(self):
        self.client.post(reverse("rider_guardian_link", args=[self.rider.pk]), {"person": self.parent_person.pk, "relationship": "Guardian"})
        link = RiderGuardian.objects.get(rider=self.rider)
        rider_person = LegacyPersonLink.objects.get(rider=self.rider).person
        response = self.client.post(reverse("rider_guardian_unlink", args=[self.rider.pk, link.pk]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_rider_card_uses_active_season_not_historical_membership(self):
        old_season, active_season = self._seasons()
        SeasonMembership.objects.create(rider=self.rider, season=old_season, team_level=SeasonMembership.TeamLevel.FUTURES)
    ...
    def test_rider_profile_separates_current_membership_from_history(self):
        old_season, active_season = self._seasons()
        SeasonMembership.objects.create(rider=self.rider, season=old_season, team_level=SeasonMembership.TeamLevel.UPPER)
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["current_membership"], current)
        self.assertEqual(list(response.context["historical_memberships"])[0].season, old_season)
        self.assertContains(response, "CURRENT IEA SEASON")
        self.assertContains(response, "IEA TEAM · CURRENT")
        self.assertContains(response, "Futures Team · Grade 7")
        self.assertContains(response, "Previous IEA seasons")
    ...
    def test_rider_profile_does_not_treat_historical_membership_as_current(self):
        ...
        response = self.client.get(reverse("rider_detail", args=[self.rider.pk]))
        self.assertEqual(response.status_code, 200)
        self.assertIsNone(response.context["current_membership"])
        self.assertContains(response, "RIDER · NOT ON CURRENT IEA TEAM")
        self.assertContains(response, f"Not assigned to {active_season.name}")
        self.assertContains(response, "2025-2026")
        self.assertNotContains(response, "IEA TEAM · CURRENT")
