...
class LessonSchedulingTests(TestCase):
    ...
    def test_generation_rejects_reversed_request_window(self):
        with self.assertRaises(ValidationError): generate_lesson_occurrences(self.series, date(2026, 10, 1), date(2026, 9, 1))
    ...
    def test_manual_occurrence_has_no_recurrence_identity(self):
        ...
        self.assertEqual(occurrence.origin, LessonOccurrence.Origin.MANUAL)
    ...
    def test_future_refresh_updates_only_uncommitted_scheduled_occurrences(self):
        ...
        self.series.name = "Tuesday Advanced"; self.series.default_location = "Outdoor Arena"; self.series.duration_minutes = 75; self.series.capacity = 4; self.series.save()
        refreshed = refresh_future_lesson_occurrences(self.series, from_datetime=timezone.make_aware(datetime(2026, 9, 20, 0, 0)))
        self.assertEqual(len(refreshed.updated), 3)
        for occurrence in result.created:
            occurrence.refresh_from_db(); self.assertEqual(occurrence.title, "Tuesday Advanced"); self.assertEqual(occurrence.capacity, 4)
    ...
    def test_future_refresh_preserves_cancelled_and_rescheduled_occurrences(self):
        ...
        self.series.default_location = "New Arena"; self.series.save()
        refreshed = refresh_future_lesson_occurrences(self.series, from_datetime=timezone.make_aware(datetime(2026, 9, 20, 0, 0)))
    ...
    def test_future_refresh_preserves_occurrence_with_attendance(self):
        ...
        self.series.default_location = "New Arena"; self.series.save()
        refreshed = refresh_future_lesson_occurrences(self.series, from_datetime=timezone.make_aware(datetime(2026, 9, 20, 0, 0)))
        self.assertEqual(len(refreshed.updated), 0); self.assertEqual(len(refreshed.preserved), 1)

    def test_future_refresh_preserves_occurrence_with_assignment(self):
        occurrence = generate_lesson_occurrences(self.series, date(2026, 9, 22), date(2026, 9, 22)).created[0]
        LessonAssignment.objects.create(occurrence=occurrence, person=self.instructor, role=LessonAssignment.Role.INSTRUCTOR)
        self.series.default_location = "New Arena"; self.series.save()
        refreshed = refresh_future_lesson_occurrences(self.series, from_datetime=timezone.make_aware(datetime(2026, 9, 20, 0, 0)))
        self.assertEqual(len(refreshed.updated), 0); self.assertEqual(len(refreshed.preserved), 1)
    ...
    def test_refresh_does_not_move_occurrence_when_series_schedule_shape_changes(self):
        ...
        self.series.weekday = 2; self.series.starts_at_time = time(18, 0); self.series.save()
        refresh_future_lesson_occurrences(self.series, from_datetime=timezone.make_aware(datetime(2026, 9, 20, 0, 0)))
        occurrence.refresh_from_db(); self.assertEqual(occurrence.starts_at, original_start)

    def test_cancel_rejects_completed_occurrence(self):
        occurrence = generate_lesson_occurrences(self.series, date(2026, 9, 22), date(2026, 9, 22)).created[0]
        occurrence.status = LessonOccurrence.Status.COMPLETED; occurrence.save()
    ...
    def test_reschedule_preserves_duration_snapshot_and_recurrence_identity(self):
        ...
        old_title, old_location, old_capacity = occurrence.title, occurrence.location, occurrence.capacity
        ...
        self.assertEqual(occurrence.scheduled_for, original_slot)
        self.assertEqual(occurrence.title, old_title); self.assertEqual(occurrence.location, old_location); self.assertEqual(occurrence.capacity, old_capacity)
    ...
    def test_generation_after_reschedule_does_not_recreate_original_slot(self):
        ...
        self.assertEqual(result.existing[0].pk, occurrence.pk)
        self.assertEqual(result.existing[0].scheduled_for, original_slot)
