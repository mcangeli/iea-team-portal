...
class FinanceWorkspaceTests(TestCase):
    def setUp(self):
        self.team=Team.objects.create(name="Workspace Barn"); self.general=ReceivableAccount.objects.create(team=self.team,name="General Account",finance_domain=FinanceDomain.GENERAL); self.iea=ReceivableAccount.objects.create(team=self.team,name="IEA Account",finance_domain=FinanceDomain.IEA)
        self.general_charge=ReceivableCharge.objects.create(account=self.general,description="Board",amount="125.00",charge_date=date(2026,9,1)); self.iea_charge=ReceivableCharge.objects.create(account=self.iea,description="Show fee",amount="75.00",charge_date=date(2026,9,2))
