...
class ShowManagerAssignment(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="show_manager_assignments")
    active = models.BooleanField(default=True)
    notes = models.CharField(max_length=255, blank=True)

    class Meta:
        ordering = ["user__last_name", "user__first_name", "user__username"]
        constraints = [models.UniqueConstraint(fields=["show", "user"], name="unique_show_manager_assignment")]
...
class HostShowOperations(models.Model):
    ...
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="created_host_show_operations")
    updated_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="updated_host_show_operations")
...
class HostShowFamilyPublication(models.Model):
    operations = models.OneToOneField(HostShowOperations, on_delete=models.CASCADE, related_name="family_publication")
    ...
    updated_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="updated_host_family_publications")
    updated_at = models.DateTimeField(auto_now=True)

    class Meta:
        verbose_name = "host show family publication"

    @property
    def selected_count(self):
        return sum(bool(value) for value in [self.publish_arrival, self.publish_parking, self.publish_warmup, self.publish_ring_operations, self.publish_hospitality, self.publish_emergency, self.publish_documents, self.publish_family_notes])

    def __str__(self):
        return f"{self.operations.show.name} — Family Information"
...
class HostShowStaffAssignment(models.Model):
    class Role(models.TextChoices):
        ...
        EMS = "ems", "EMS"
        OTHER = "other", "Other"
    operations = models.ForeignKey(HostShowOperations, on_delete=models.CASCADE, related_name="staff_assignments")
...
class HostShowReadinessCheckpoint(models.Model):
    ...
    operations = models.ForeignKey(HostShowOperations, on_delete=models.CASCADE, related_name="readiness_checkpoints")
    title = models.CharField(max_length=180)
    due_at = models.DateTimeField(null=True, blank=True)
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="host_show_readiness_checkpoints")
...
class HostShowDutyAssignment(models.Model):
    ...
    class Status(models.TextChoices):
        ...
        COMPLETE = "complete", "Complete"
    operations = models.ForeignKey(HostShowOperations, on_delete=models.CASCADE, related_name="duty_assignments")
    area = models.CharField(max_length=20, choices=Area.choices, default=Area.OTHER)
    title = models.CharField(max_length=180)
    assigned_user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="host_show_duties")
    assigned_name = models.CharField(max_length=160, blank=True, help_text="Use for a volunteer or staff member without a portal login.")
    ...
    relieved_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="host_show_duty_handoffs")
    sort_order = models.PositiveIntegerField(default=0)
    class Meta:
