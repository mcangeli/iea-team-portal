"""v3.2.3 compatibility refinements for rider roster and family linking."""
...
from portal.view_modules.common import _can_manage, _require_manage, _selected_team
...
def rider_list(request):
    ...
    if season:
        futures = qs.filter(
            ...
            memberships__team_level=SeasonMembership.TeamLevel.FUTURES,
        ).distinct()
        upper = qs.filter(
            memberships__season=season,
            memberships__team_level=SeasonMembership.TeamLevel.UPPER,
        ).distinct()
...
def rider_guardian_add(request, pk):
    ...
    return render(
        request,
        "portal/form.html",
        {"form": form, "title": f"Add parent/guardian · {rider.display_name}", "eyebrow": "FAMILY CONTACT"},
    )
...
def rider_guardian_edit(request, pk, guardian_pk):
    ...
    form = GuardianContactForm(
        request.POST or None,
        instance=link.guardian,
        initial={"relationship": link.relationship, "primary_contact": link.primary_contact},
    )
    if form.is_valid():
        try:
            with transaction.atomic():
    ...
    return render(
        request,
        "portal/form.html",
        {"form": form, "title": f"Edit {link.guardian.display_name}", "eyebrow": "FAMILY CONTACT"},
    )
...
def rider_guardian_link(request, rider_pk):
    ...
    candidates = (
        ...
        .exclude(pk=rider_person.pk)
        .exclude(pk__in=linked_person_ids)
        ...
        try:
            with transaction.atomic():
                ...
                link, created = RiderGuardian.objects.get_or_create(
                    rider=rider,
                    guardian=guardian,
                    defaults={"relationship": relationship, "primary_contact": primary_contact},
                )
                if not created:
    ...
    return render(
        request,
        "portal/rider_guardian_link.html",
        {"rider": rider, "people": candidates},
    )
