...
from ..services.calendar_items_v340 import FILTER_CHOICES, VALID_FILTERS, calendar_items
...
def calendar(request):
    ...
    events = calendar_items(
        ...
        include_private=_can_manage(request.user),
    )
    ...
    month_weeks = []
    for week in pycalendar.Calendar(firstweekday=6).monthdatescalendar(selected_month.year, selected_month.month):
        month_weeks.append([{"date": day, "in_month": day.month == selected_month.month, "is_today": day == today, "events": events_by_day.get(day, [])} for day in week])
