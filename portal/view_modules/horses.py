"""Horse registry, show horse assignments, and Horse of the Day views."""
...
from ..horse_forms import HorseCogginsRecordForm, HorseForm, HorseSeasonProfileForm, HorseShowAssignmentForm, HorseShowAwardForm
...
def _horse_for_user(user, pk):
    return get_object_or_404(Horse, pk=pk, team=_team(user))
...
def horse_list(request):
    ...
    return render(request, "portal/horse_list.html", {"horses": horses, "can_manage": _can_manage(request.user)})
...
def horse_detail(request, pk):
    ...
    return render(request, "portal/horse_detail.html", {
        ...
        "show_assignments": horse.show_assignments.select_related("show").prefetch_related("show_classes"),
        "awards": HorseShowAward.objects.filter(assignment__horse=horse).select_related("show", "assignment").order_by("-show__show_date"),
...
def horse_create(request):
    ...
    if form.is_valid():
        ...
        messages.success(request, f"{horse.display_name} was added to the Horse Registry."); return redirect("horse_detail", pk=horse.pk)
...
def horse_edit(request, pk):
    ...
    if form.is_valid():
        horse = form.save(); _audit_event(team=horse.team, actor=request.user, action=AuditEvent.Action.UPDATED, obj=horse, summary=f"Updated horse {horse.display_name}")
        messages.success(request, f"{horse.display_name} was updated."); return redirect("horse_detail", pk=horse.pk)
...
def horse_coggins_add(request, horse_pk):
    ...
    form = HorseCogginsRecordForm(request.POST or None, request.FILES or None)
    if form.is_valid():
        record = form.save(commit=False); record.horse = horse; record.save()
...
def horse_coggins_edit(request, horse_pk, pk):
    ...
    form = HorseCogginsRecordForm(request.POST or None, request.FILES or None, instance=record)
    if form.is_valid():
        record = form.save(); _audit_event(team=horse.team, actor=request.user, action=AuditEvent.Action.UPDATED, obj=record, summary=f"Updated Coggins record for {horse.display_name}")
...
def show_horses(request, show_pk):
    ...
    return render(request, "portal/show_horses.html", {
        "show": show, "rows": rows, "awards": awards,
        "can_manage": _can_manage_show_horses(request.user, show), "can_manage_registry": _can_manage(request.user),
    })


@login_required
def show_horse_add(request, show_pk):
    team = _team(request.user); show = get_object_or_404(Show.objects.select_related("season"), pk=show_pk, team=team); _require_show_horse_manage(request.user, show); _ensure_season_open(show.season)
    allow_override = _can_manage(request.user); form = HorseShowAssignmentForm(request.POST or None, show=show, allow_eligibility_override=allow_override)
    if form.is_valid():
...
def show_horse_edit(request, show_pk, pk):
    team = _team(request.user); show = get_object_or_404(Show.objects.select_related("season"), pk=show_pk, team=team); _require_show_horse_manage(request.user, show); _ensure_season_open(show.season)
    assignment = get_object_or_404(HorseShowAssignment, pk=pk, show=show); allow_override = _can_manage(request.user)
...
def show_horse_remove(request, show_pk, pk):
    ...
    assignment.delete(); messages.success(request, f"{label} was removed from this show."); return redirect("show_horses", show_pk=show.pk)
...
def show_horse_award_add(request, show_pk):
    ...
    if form.is_valid():
        award = form.save(commit=False); award.show = show; award.save(); _audit_event(team=team, actor=request.user, action=AuditEvent.Action.CREATED, obj=award, season=show.season, summary=f"Recorded {award.get_session_display()} Horse of the Day for {award.horse.display_name}")
        messages.success(request, f"{award.get_session_display()} Horse of the Day recorded for {award.horse.display_name}."); return redirect("show_horses", show_pk=show.pk)
...
def show_horse_award_edit(request, show_pk, pk):
    team = _team(request.user); show = get_object_or_404(Show.objects.select_related("season"), pk=show_pk, team=team); _require_show_horse_manage(request.user, show); _ensure_season_open(show.season)
    award = get_object_or_404(HorseShowAward, pk=pk, show=show); form = HorseShowAwardForm(request.POST or None, instance=award, show=show)
    if form.is_valid():
        award = form.save(); _audit_event(team=team, actor=request.user, action=AuditEvent.Action.UPDATED, obj=award, season=show.season, summary=f"Updated {award.get_session_display()} Horse of the Day")
...
def show_horse_award_remove(request, show_pk, pk):
    ...
    award.delete(); messages.success(request, "Horse of the Day award removed."); return redirect("show_horses", show_pk=show.pk)
