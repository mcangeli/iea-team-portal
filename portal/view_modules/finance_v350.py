...
from portal.forms_v350_finance import FinanceAccountForm, FinanceAllocationForm, FinanceChargeForm, FinanceCreditForm, FinancePaymentForm
...
def _domain_summary(user, team, domain):
    rows = list(finance_accounts_for_user(user, team).filter(finance_domain=domain).select_related("primary_person"))
    return {"domain": domain, "label": "General Barn" if domain == FinanceDomain.GENERAL else "IEA", "accounts": rows, "account_count": len(rows),
        "balance": sum((a.balance for a in rows), ZERO), "amount_due": sum((a.amount_due for a in rows), ZERO),
        "unapplied_payments": sum((a.unapplied_payment_total for a in rows), ZERO), "unapplied_credits": sum((a.unapplied_credit_total for a in rows), ZERO)}


@login_required
def finance_workspace(request):
    team = _team_for_finance_user(request.user); domains = allowed_finance_domains(request.user, team)
    summaries = [_domain_summary(request.user, team, d) for d in (FinanceDomain.GENERAL, FinanceDomain.IEA) if d in domains]
    accounts = finance_accounts_for_user(request.user, team).select_related("primary_person").order_by("finance_domain", "name")
    return render(request, "portal/finance_workspace_v350.html", {"team": team, "domain_summaries": summaries, "accounts": accounts, "can_see_general": FinanceDomain.GENERAL in domains, "can_see_iea": FinanceDomain.IEA in domains})


@login_required
def finance_receivable_account_add(request):
    team = _team_for_finance_user(request.user); domains = allowed_finance_domains(request.user, team)
    form = FinanceAccountForm(request.POST or None, team=team, allowed_domains=domains)
    if request.method == "POST" and form.is_valid():
        try: account = create_account_for_user(request.user, team=team, **form.cleaned_data)
        except ValidationError as exc: form.add_error(None, exc)
        else:
            messages.success(request, "Receivable account created.")
...
def _operation_form(request, pk, form_class, title, submit_label, operation):
    team, account = _account_for_request(request, pk); form = form_class(request.POST or None, initial={"charge_date": date.today(), "received_date": date.today(), "credit_date": date.today()})
    if request.method == "POST" and form.is_valid():
        try: operation(request.user, account.pk, team=team, **form.cleaned_data)
        except ValidationError as exc: form.add_error(None, exc)
        else:
            messages.success(request, f"{submit_label} saved."); return redirect("finance_receivable_account_detail", pk=account.pk)
...
def finance_receivable_statement(request, pk):
    ...
    try:
        start_date = date.fromisoformat(request.GET.get("start")) if request.GET.get("start") else date(today.year, 1, 1); end_date = date.fromisoformat(request.GET.get("end")) if request.GET.get("end") else today
