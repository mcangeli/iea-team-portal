...
def _require_active_account(account):
    if account.status != ReceivableAccount.Status.ACTIVE: raise ValidationError("Closed receivable accounts are read-only.")
    return account
...
def add_account_person_for_user(user,account_id,*,person,role,statement_recipient=False,notes="",team=None):
    account=_require_active_account(_authorized_account(user,account_id,team))
...
def create_charge_for_user(user,account_id,*,description,amount,charge_date,due_date=None,charge_type="",season=None,notes="",team=None):
    account=_require_active_account(_authorized_account(user,account_id,team));charge=ReceivableCharge(account=account,description=description.strip(),amount=Decimal(amount),charge_date=charge_date,due_date=due_date,charge_type=charge_type.strip(),season=season,notes=notes.strip());charge.full_clean();charge.save();return charge
@transaction.atomic
...
def post_credit_for_user(user,account_id,*,description,amount,credit_date,charge_id=None,credit_type="",notes="",season=None,team=None):
    account=_require_active_account(_authorized_account(user,account_id,team));charge=_posted_charge(account,charge_id) if charge_id else None;return post_credit(account=account,description=description.strip(),amount=Decimal(amount),credit_date=credit_date,charge=charge,credit_type=credit_type.strip(),notes=notes.strip(),season=season)
@transaction.atomic
def allocate_payment_for_user(user,account_id,*,payment_id,charge_id,amount=None,notes="",team=None):
    account=_require_active_account(_authorized_account(user,account_id,team));charge=_posted_charge(account,charge_id)
