...
from portal.model_modules.finance import ReceivableAccount, ReceivableCharge
...
def _posted_charge(account, charge_id):
    try:
        return account.charges.get(pk=charge_id, status=ReceivableCharge.Status.POSTED)
...
def create_account_for_user(user, *, team, name, finance_domain, primary_person=None, notes=""):
    if not can_manage_finance_domain(user, finance_domain, team):
        raise PermissionDenied
    account = ReceivableAccount(team=team, name=name.strip(), finance_domain=finance_domain, primary_person=primary_person, notes=notes.strip())
    account.full_clean()
    account.save()
    return account
...
def create_charge_for_user(user, account_id, *, description, amount, charge_date, due_date=None, charge_type="", season=None, notes="", team=None):
    ...
    charge = ReceivableCharge(account=account, description=description.strip(), amount=Decimal(amount), charge_date=charge_date, due_date=due_date, charge_type=charge_type.strip(), season=season, notes=notes.strip())
    charge.full_clean(); charge.save(); return charge
...
def post_payment_for_user(user, account_id, *, amount, received_date, charge_id=None, method="", reference="", notes="", deposit_account=None, income_category=None, season=None, team=None):
    ...
    return post_payment(account=account, amount=Decimal(amount), received_date=received_date, charge=charge, method=method.strip(), reference=reference.strip(), notes=notes.strip(), deposit_account=deposit_account, income_category=income_category, season=season)
...
def post_credit_for_user(user, account_id, *, description, amount, credit_date, charge_id=None, credit_type="", notes="", season=None, team=None):
    account = _authorized_account(user, account_id, team)
    charge = _posted_charge(account, charge_id) if charge_id else None
    return post_credit(account=account, description=description.strip(), amount=Decimal(amount), credit_date=credit_date, charge=charge, credit_type=credit_type.strip(), notes=notes.strip(), season=season)
