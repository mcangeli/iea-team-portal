...
def update_lesson_day(occurrence: LessonOccurrence, rows: list[LessonDayRowUpdate]) -> LessonDayUpdateResult:
    ...
    horses = {horse.pk: horse for horse in Horse.objects.filter(team_id=team_id, pk__in=horse_ids)}
    if set(horses) != horse_ids:
        raise ValidationError("One or more selected horses are not available to this organization.")
