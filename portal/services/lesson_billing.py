...
@transaction.atomic
def bill_lesson_occurrence(*,occurrence:LessonOccurrence,rule:ReceivableBillingRule,person_accounts=None):
    """Bill billable attendance through the generic service-charge boundary.

    Explicit person_accounts may be supplied by an authorized caller. Otherwise
    participant links resolve the account without guessing family ownership.
    """
    ...
    for attendance in rows:
        ...
        account=person_accounts.get(attendance.person_id) if person_accounts is not None else resolve_participant_account(attendance.person,finance_domain=rule.account.finance_domain)
        if account is None:
            skipped.append(attendance);continue
