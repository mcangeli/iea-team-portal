...
def has_active_parent_relationship(parent_person, child_person):
    if not parent_person or not child_person or parent_person.team_id != child_person.team_id:
        return False
    return PersonRelationship.objects.filter(from_person=parent_person, to_person=child_person,
        relationship_type=PersonRelationship.RelationshipType.PARENT_GUARDIAN, active=True, end_date__isnull=True).exists()


def can_view_private_person(user, person):
    if not getattr(user, "is_authenticated", False): return False
    if can_manage_people(user): return True
    viewer = _person_for_login(user)
    if viewer and viewer.pk == person.pk: return True
    return has_active_parent_relationship(viewer, person)


def canonical_rider_ids_for_parent(user, team):
    viewer = _person_for_login(user)
    if not viewer or viewer.team_id != team.id: return []
    child_person_ids = PersonRelationship.objects.filter(from_person=viewer, to_person__team=team,
        relationship_type=PersonRelationship.RelationshipType.PARENT_GUARDIAN, active=True,
        end_date__isnull=True).values_list("to_person_id", flat=True)
    return list(LegacyPersonLink.objects.filter(person_id__in=child_person_ids, rider__team=team,
        rider__isnull=False).values_list("rider_id", flat=True))
...
def canonical_self_rider_ids_for_user(user, team):
    ...
    viewer = _person_for_login(user)
    if not viewer or viewer.team_id != team.id: return []
    return list(LegacyPersonLink.objects.filter(person=viewer, rider__team=team,
        rider__isnull=False).values_list("rider_id", flat=True))
...
def personal_riders_for_user(user, team):
    ...
    return team.riders.filter(Q(id__in=parent_ids) | Q(id__in=self_ids) | Q(user=user)
        | Q(guardians=user) | Q(guardian_links__guardian__user=user)).distinct()


def visible_riders_for_user(user, team):
    qs = team.riders.all(); profile = getattr(user, "profile", None)
    if profile and profile.role == profile.Role.PARENT: return personal_riders_for_user(user, team)
...
def can_view_private_rider(user, rider):
    ...
    if rider.user_id == user.id: return True
    viewer = _person_for_login(user)
    try: rider_person = rider.person_bridge.person
    except LegacyPersonLink.DoesNotExist: rider_person = None
    if viewer and rider_person and viewer.pk == rider_person.pk: return True
    if has_active_parent_relationship(viewer, rider_person): return True
...
def can_view_family_account(user, membership):
    ...
    if _can_finance(user, membership.season): return True
    rider = membership.rider; viewer = _person_for_login(user)
    try: rider_person = rider.person_bridge.person
    except LegacyPersonLink.DoesNotExist: rider_person = None
    if has_active_parent_relationship(viewer, rider_person): return True
    return rider.guardians.filter(pk=user.pk).exists() or rider.guardian_links.filter(guardian__user=user).exists()
...
def require_people_manager(user):
    if not can_manage_people(user): raise PermissionDenied